...
class TestIterativeSurveySystem(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.system = IterativeSurveySystem(
            claude_wrapper=self.mock_wrapper,
            max_iterations=3
        )
    ...
    def test_initialization(self):
        ...
        self.assertIsNotNone(self.system.baseline)
    ...
    def test_convergence_detection(self):
        ...
        # Mock baseline and improver
        self.system.baseline.generate_survey = Mock(return_value={'sections': []})
        self.system.improver.improve_survey = Mock(return_value={'sections': []})
        
        # Run system
        papers = [{'title': 'Paper 1'}]
        survey, iterations = self.system.generate_iterative_survey(papers)
        
        # Should converge immediately after first verification
        self.assertEqual(len(iterations), 1)
        self.assertEqual(iterations[0]['overall'], 4.2)
    ...
    def test_max_iterations_limit(self):
        ...
        self.assertEqual(len(iterations), 3)  # max_iterations = 3
