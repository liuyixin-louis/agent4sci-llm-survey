...
Make it natural and maintain flow."""
    ...
    def _improve_citations(self, survey: Dict, papers: List[Dict]) -> Dict:
        """Add missing citations to unsupported claims."""
        logger.info("Improving citations")
        
        # Extract paper titles and abstracts for matching
        paper_context = []
        for paper in papers[:30]:  # Limit to prevent token overflow
            paper_context.append(f"Title: {paper.get('title', '')}")
            if 'abstract' in paper:
                paper_context.append(f"Abstract: {paper['abstract'][:200]}...")
        ...
        for i, section in enumerate(sections):
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are an expert at identifying unsupported claims and adding appropriate citations."
                },
                {
                    "role": "user",
                    "content": f"""Improve citations in this section by:
1. Identifying claims that lack citations
2. Adding [Author, Year] style citations where needed
3. Ensuring all factual statements are supported
...
Return the improved section with proper citations added. Keep the same structure and content, just add missing citations."""
                }
            ]
            ...
            try:
                response = self.claude_wrapper.chat_completion(
                    messages=messages,
                    model="sonnet",  # Better model for citation accuracy
                    use_cache=True
                )
                ...
                if isinstance(response, dict) and "choices" in response:
                    ...
                    logger.warning(f"Unexpected response format for citations: {type(response)}")
                    continue
                ...
                logger.info(f"Improved citations in section: {title}")
                
            except Exception as e:
                logger.error(f"Error improving citations for section {title}: {e}")
                continue
        
        return survey
        
    def _improve_structure(self, survey: Dict) -> Dict:
        """Improve survey organization and logical flow."""
        logger.info("Improving structure")
        
        # Get current section titles and summaries
        sections = survey.get('sections', [])
        section_summaries = []
        ...
        for section in sections:
            ...
            section_summaries.append(f"{title}: {content}...")
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an expert at organizing scientific surveys for optimal logical flow and readability."
            },
            {
                "role": "user",
                "content": f"""Analyze this survey structure and suggest improvements:

Current Structure:
{current_structure}

Identify and fix:
1. Sections that should be reordered for better flow
2. Sections that could be merged if redundant
3. Missing transitions between major themes
4. Any organizational issues

Provide specific instructions for restructuring. If the structure is already good, say "No changes needed"."""
            }
        ]
        ...
        try:
            ...
            if isinstance(response, dict) and "choices" in response:
                ...
                logger.warning(f"Unexpected response format for structure: {type(response)}")
                return survey
            
            # If no changes needed, return as is
            if "no changes needed" in suggestions.lower():
                logger.info("Structure already optimal")
                return survey
            
            # Apply suggested reordering if mentioned
            if "reorder" in suggestions.lower() or "move" in suggestions.lower():
                # Parse suggestions to identify reordering
                # For now, ensure Introduction is first and Conclusion is last
                intro_idx = None
                conclusion_idx = None
                
                for i, section in enumerate(sections):
                    title = section.title if isinstance(section, SurveySection) else section.get('title', '')
                    if 'introduction' in title.lower():
                        intro_idx = i
                    elif 'conclusion' in title.lower():
                        conclusion_idx = i
                
                # Move introduction to start if not already there
                if intro_idx is not None and intro_idx != 0:
                    sections.insert(0, sections.pop(intro_idx))
                    logger.info("Moved Introduction to beginning")
                
                # Move conclusion to end if not already there
                if conclusion_idx is not None and conclusion_idx != len(sections) - 1:
                    sections.append(sections.pop(conclusion_idx))
                    logger.info("Moved Conclusion to end")
            
            # Add section numbering for clarity
            for i, section in enumerate(sections):
                if isinstance(section, SurveySection):
                    if not section.title.startswith(f"{i+1}."):
                        section.title = f"{i+1}. {section.title}"
                else:
                    if not section.get('title', '').startswith(f"{i+1}."):
                        section['title'] = f"{i+1}. {section.get('title', '')}"
            
            survey['sections'] = sections
            logger.info("Structure improved with reordering and numbering")
